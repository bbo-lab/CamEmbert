...
color_master_$i_$o: $(DATE)
	sleep 1
	python3 main.py -i $i -o $(DATE)/$o.mp4 -t $(TIME) -g $(DATE)/$o.csv -f 90 -m $(TRIGGER) -c

color_slave_$i_$o: $(DATE)
	python3 main.py -i $i -o $(DATE)/$o.mp4 -t $(TIME) -g $(DATE)/$o.csv -f 90 -s $(TRIGGER) -c
endef
...
gray_master_$i_$o: $(DATE)
	sleep 1
	python3 main.py -i $i -o $(DATE)/$o.mp4 -t $(TIME) -g $(DATE)/$o.csv -f 150 -m $(TRIGGER) --exposure 5000

gray_slave_$i_$o: $(DATE)
	python3 main.py -i $i -o $(DATE)/$o.mp4 -t $(TIME) -g $(DATE)/$o.csv -f 150 -s $(TRIGGER) --exposure 5000
endef
$(foreach o, 0 1 2 3, $(foreach i,0 1,$(eval $(call make_capture_in_out_target))))
